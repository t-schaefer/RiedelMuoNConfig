...
def probes(ip):
    ...
    return [
        ...
        # mdns_enable and igmp.version are deliberately not probed: on the
        # MuoN test card writing them was followed by a reboot (2026-09-25).
        ...
        ("sdi v_offset", sdi, ["line_offset", "v_offset"], lambda v: "1" if str(v) != "1" else "0"),
...
def main():
    ...
    for name, path, field, change in probes(ip):
        try:
            ...
            print("\nStopped: the card stopped answering. Check its uptime (self/system) - it may be rebooting -\n"
                  "and whether this probe's field kept the test value before running again.")
            return
